...
internal class LookDownRemoteImpl(
  ...
  override suspend fun setup(url:String, resume:Boolean, file: File, headers: Map<String, String>?): Pair<Boolean,String> {
    kotlin.runCatching {
      this.connection = URL(url).openConnection() as HttpURLConnection
      ...
      if (connection.responseCode != HttpURLConnection.HTTP_OK && connection.responseCode != HttpURLConnection.HTTP_PARTIAL) {
        return Pair(false, getHttpMessage())
      }
    }
    return Pair(true, getHttpMessage())
  }
