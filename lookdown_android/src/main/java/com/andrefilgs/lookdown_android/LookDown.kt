...
class LookDown(private val context: Context,
  ...
  class Builder(private val context: Context){
    ...
    fun build(): LookDown {
      return LookDown(context, id=id, driver= driver, folder= folder, resume= forceResume, headers= headers,
                      chunkSize= chunkSize, timeout= timeout, connectionTimeout=connectTimeout, fileExtension=fileExtension, progressRenderDelay= progressRenderDelay)
    }
  }
  ...
  suspend fun download(urlStr: String,
    ...
    flow {
      ...
      if (file.exists() && resume.orDefault()) {
        tempFileExists = true
      ...
      try {
        ...
        if (ldDownload.state == LDDownloadState.Downloaded) {
          ...
          Fileman.removeTempExtension(file,
